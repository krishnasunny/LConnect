...
export function LawyersGrid() {
  const [visibleLawyers, setVisibleLawyers] = useState(6)

  const loadMore = () => {
    setVisibleLawyers((prev) => Math.min(prev + 3, lawyersList.length))
  }

  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {lawyersList.slice(0, visibleLawyers).map((lawyer) => (
          <LawyerCard key={lawyer.id} lawyer={lawyer} />
        ))}
      </div>

      {visibleLawyers < lawyersList.length && (
